...
{   	
	...
	// SYN7318_Init();
		 ...
		 SYN_TTS((uint8_t*)"语音识别测试,请发语音唤醒词，语音驾驶");
		  LED1 = 1;
...
//															Back_Test(50,50);
													 		SYN_TTS((uint8_t*)"小车后退已完成，等待下一步指令");
														  SYN7318_Put_String(Stop_ASR_Buf,4); //停止语音识别 
															break;
													}
													case 0x03:
...
//														  Left_Test( 80);
															SYN_TTS((uint8_t*)"小车左转完成，等待下一步指令");
														  SYN7318_Put_String(Stop_ASR_Buf,4); //停止语音识别;
															break;
													}
													case 0x04:
...
//															Right_Test( 80);
															SYN_TTS((uint8_t*)"小车右转完成，等待下一步指令");
														  SYN7318_Put_String(Stop_ASR_Buf,4); //停止语音识别
															break;
													}
													case 0x05:
...
//															  Track_Test( 50);
													...
													case 0x07:
													{
														    ...
														    SYN_TTS((uint8_t*)"报警器已打开，等待下一步指令");
														    SYN7318_Put_String(Stop_ASR_Buf,4); //停止语音识别
															break;
													}
													case 0x08:
													...
													case 0x0a:
													{
														    ...
														    SYN_TTS((uint8_t*)"LED显示已开，等待下一步指令");
																SYN7318_Put_String(Stop_ASR_Buf,4); //停止语音识别										
															  break;
													}
													case 0x0b:
													{
														    ...
														    SYN_TTS((uint8_t*)"计时系统已打开，等待下一步指令");
 														    SYN7318_Put_String(Stop_ASR_Buf,4); //停止语音识别
															  break;
													}
													case 0x0c:
													{
														  ...
														  SYN_TTS((uint8_t*)"计时系统已关闭，等待下一步指令");
                              SYN7318_Put_String(Stop_ASR_Buf,4); //停止语音识别
															break;
													}
													case 0x0d:
													{
														  ...
														  SYN_TTS((uint8_t*)"LED显示距离已完成，等待下一步指令");
                                  SYN7318_Put_String(Stop_ASR_Buf,4); //停止语音识别
															break;
													}
													case 0x0e:
													{
									//						LED_L =0;  // 开左灯
														  SYN_TTS((uint8_t*)"左侧提示灯已打开，等待下一步指令");
														SYN7318_Put_String(Stop_ASR_Buf,4); //停止语音识别
													...
													{
									//						LED_R =0;  // 开右灯
														  SYN_TTS((uint8_t*)"右侧提示灯已打开，等待下一步指令");
														  SYN7318_Put_String(Stop_ASR_Buf,4); //停止语音识别														
															break;
													}
													case 0x10:
									...
									//						LED_L =1;  // 关闭提示灯
									//						LED_R =1;
														  SYN_TTS((uint8_t*)"提示灯已关闭，等待下一步指令");
                              SYN7318_Put_String(Stop_ASR_Buf,4); //停止语音识别
															break;
													}
													case 0x11:
									...
									//						BEEP =0;  //开蜂鸣器
														  SYN_TTS((uint8_t*)"蜂鸣器已打开，等待下一步指令");
                              ...
                              SYN7318_Put_String(Stop_ASR_Buf,4); //停止语音识别
									...
									//						BEEP =1;  //关蜂鸣器
														  SYN_TTS((uint8_t*)"蜂鸣器已关闭，等待下一步指令");
														  SYN7318_Put_String(Stop_ASR_Buf,4); //停止语音识别
															break;
													}
													case 0x13:
													...
													case 0x14:
													{
														   ...
														   SYN_TTS((uint8_t*)"隧道排风系统已打开，等待下一步指令");
														  SYN7318_Put_String(Stop_ASR_Buf,4); //停止语音识别
													...
													case 0x15:
													{
														  ...
														  SYN_TTS((uint8_t*)"图片翻页完成，等待下一步指令");
														  SYN7318_Put_String(Stop_ASR_Buf,4); //停止语音识别
													...
													case 0x16:
													{
														  ...
														  SYN_TTS((uint8_t*)"调[=tiao2]光档[=dang3]位已加1，等待下一步指令");
                               SYN7318_Put_String(Stop_ASR_Buf,4); //停止语音识别
															break;
													}
													case 0x17:
													{
														  ...
														  SYN_TTS((uint8_t*)"欢迎使用北京百科融[=rong2]创语音智能小车");
                               SYN7318_Put_String(Stop_ASR_Buf,4); //停止语音识别					
															break;
													}
													case 0x18:
...
/*															while(G_Flag);  //等待前进完成
															...
															Left_Test( 80); 
															while(L_Flag);*/
															SYN_TTS((uint8_t*)"原地掉头已完成，等待下一步指令");
                              SYN7318_Put_String(Stop_ASR_Buf,4); //停止语音识别	
															break;
													}
													case 0x19:
...
//															Go_Test( 80,10);
															SYN_TTS((uint8_t*)"小车前进已完成，等待下一步指令");
															SYN7318_Put_String(Stop_ASR_Buf,4); //停止语音识别
															break;
													}
													case 0x1B:   //  小车后退
...
//														   Left_Test( 80);
															 SYN_TTS((uint8_t*)"小车左转已完成，等待下一步指令");
                               SYN7318_Put_String(Stop_ASR_Buf,4); //停止语音识别;
															break;
													}
													case 0x1D:  // 小车右转
...
//															Right_Test( 80);
															SYN_TTS((uint8_t*)"小车右转已完成，等待下一步指令");
														SYN7318_Put_String(Stop_ASR_Buf,4); //停止语音识别
...
//															Track_Test( 50);
															SYN_TTS((uint8_t*)"小车寻迹已完成，等待下一步指令");
                              SYN7318_Put_String(Stop_ASR_Buf,4); //停止语音识别
															break;
													}	
                          case 0x20:   //  向右转弯
...
//															Right_Test( 80);
															SYN_TTS((uint8_t*)"右转弯已完成，等待下一步指令");
														  SYN7318_Put_String(Stop_ASR_Buf,4); //停止语音识别
															break;
													}														
                          case 0x21:   //  禁止右转
...
//															Left_Test( 80);
															SYN_TTS((uint8_t*)"禁止右转，执行左转弯完成，等待下一步指令");
                              SYN7318_Put_String(Stop_ASR_Buf,4); //停止语音识别
															break;
														}
                          case 0x22:   //  向左转弯
...
//															Left_Test( 80);
															SYN_TTS((uint8_t*)"左转弯已完成，等待下一步指令");
                              SYN7318_Put_String(Stop_ASR_Buf,4); //停止语音识别
															break;
													}														
                          case 0x23:   //  禁止右转
...
//															Left_Test( 80);
															SYN_TTS((uint8_t*)"禁止左转，执行右转弯完成，等待下一步指令");
                              SYN7318_Put_String(Stop_ASR_Buf,4); //停止语音识别
															break;													
													}
													default:
...
//												Ysn7813_flag_cs = 0;
											...
											case 0x04:
											{
												SYN_TTS((uint8_t*)"请安静，麻烦您再说一遍");
												SYN7318_Put_String(Stop_ASR_Buf,4); //停止语音识别
												break;
											}
											case 0x05:
											{
												SYN_TTS((uint8_t*)"对不起，请再说一遍");
												SYN7318_Put_String(Stop_ASR_Buf,4); //停止语音识别
												break;
											}
											case 0x06:
											{
												SYN_TTS((uint8_t*)"识别内部错误");
												SYN7318_Put_String(Stop_ASR_Buf,4); //停止语音识别
												break;
											}
											case 0x07:
...
unsigned char Start_ASR_One_Buf[] = {0xFD,0x00,0x02,0x10,0x04};//  词典 04-用户2  对应 0x04
...
//			 SYN_TTS("模块复位成功！");
		...
		if(Back[3] == 0x41)           //接收成功
		{
			...
			if(Back[0] == 0xfc)        //帧头判断
			{
				...
				if(ASR[0] ==0x01) 
				{
					Speech_recognition_data[2]=ASR[5];
					switch(ASR[5]) // 命令ID

					{
								case 0x02:  
					 {
							  ...
							  SYN_TTS("美好生活");
							  SYN7318_Put_String(Stop_ASR_Buf,4); //停止语音识别
							  Speech_recognition_data[2] = 0x06;
							  LTXS_YY[2]=0x32;					 
							  Ysn7813_flag =0;
							 break;
						}														
						...
						{								 
							...
							SYN_TTS("秀丽山河");								
							 SYN7318_Put_String(Stop_ASR_Buf,4); //停止语音识别			
							 Speech_recognition_data[2] = 0x02;
						   	 LTXS_YY[2]=0x33;
							 Ysn7813_flag =0;
							 break;
						}														
						...
						{														  															
							...
							SYN_TTS("追逐梦想");
							SYN7318_Put_String(Stop_ASR_Buf,4); //停止语音识别	
							Speech_recognition_data[2] = 0x03;
							LTXS_YY[2]=0x34;
							Ysn7813_flag =0;
							break;
						}
                        case 0x05:  
						{
							...
							SYN_TTS("扬帆启航");
							SYN7318_Put_String(Stop_ASR_Buf,4); //停止语音识别
							Speech_recognition_data[2] = 0x04;
					 		LTXS_YY[2]=0x35;
							Ysn7813_flag =0;
							break;
						}														
						...
						{														  	
							...
							SYN_TTS("齐头并进");
							SYN7318_Put_String(Stop_ASR_Buf,4); //停止语音识别	
							Speech_recognition_data[2] = 0x05;
							LTXS_YY[2]=0x36; 							
							Ysn7813_flag =0;
							break;													
						}
