...
void Ultrasonic_EXTI()
{
	...
	SYSCFG_EXTILineConfig(INT0_EXTI_Port,INT0_EXTI_Pin);
	
	EXTI_InitStructure.EXTI_Line = INT0_EXTI_Line;
	EXTI_InitStructure.EXTI_Mode = EXTI_Mode_Interrupt;
	EXTI_InitStructure.EXTI_Trigger = EXTI_Trigger_Falling;
	EXTI_InitStructure.EXTI_LineCmd = ENABLE;
	EXTI_Init(&EXTI_InitStructure);
	
	NVIC_InitStructure.NVIC_IRQChannel = INT0_EXTI_IRQn;
	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 0;
...
void INT0_EXTI_IRQHandler(void)
{
	if(EXTI_GetITStatus(INT0_EXTI_Line) == SET)
	{
		if(GPIO_ReadInputDataBit(INT0_Prot,INT0_Pin) == RESET)
		{	
			TIM_Cmd(TIM6,DISABLE);
			Ultrasonic_Value = Ultrasonic_Num;
			Ultrasonic_Value =(double)Ultrasonic_Num*1.72-20;       // 计算距离定时10us，S=Vt/2（减2是误差补尝）
			dis = (uint16_t) Ultrasonic_Value;
		}
		EXTI_ClearITPendingBit(INT0_EXTI_Line);
	}
}
