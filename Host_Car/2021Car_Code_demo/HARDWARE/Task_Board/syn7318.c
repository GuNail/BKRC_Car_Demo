...
static uint8_t YY_ZD[8]={0x55 ,0x06 ,0x10 ,0x00 ,0x00 ,0x00 ,0x00,0xbb};  // 左转弯
static uint8_t YY_SJ [8]={0x55 ,0x06 ,0x20 ,0x01 ,0x00 ,0x00 ,0x21 ,0xbb}; 
    ...
    sprintf((char*)str_test,"%d",S[3]);
		...
		if(Back[3] == 0x41)           //接收成功
		{
			...
			else  
			{
				YY_ZD[3] = num;
				_Checksum_(YY_ZD);
				Send_ZigbeeData_To_Fifo( YY_ZD,8);
			}
			SYN7318_Get_String(Back,3);  //语音识别命令回传结果
			if(Back[0] == 0xfc)        //帧头判断
			{
				...
				if(ASR[0] ==0x01)
				{
					switch(ASR[5]) // 命令ID
					{
						...
						default:
						{														  
							SYN_TTS((uint8_t*)"对不起，我没听清,再发一次");
							SYN7318_Put_String(Stop_ASR_Buf,4); //停止语音识别
							break;
						}
					}
				}
				else SYN_TTS((uint8_t*)"识别失败，再来一次！");     
			}
		}			
		LED3 = !LED3;
		error_num++;	
...
void SYN_Voice(u8 Mode,_SYN_VoiceType Voice)
{
	...
	while(SYN_Flag)
	{
		...
		delay_ms(500);
		SYN7318_Put_String(Start_ASR_One_Buf,5);
		SYN7318_Get_String(Back,4); 
		if(Back[3] == 0x41)           //接收成功
		{
			LED1 = ~LED1;             //LED1反转
			...
			else  
			{
				YY_ZD[3] = Voice;
				_Checksum_(YY_ZD);
				Send_ZigbeeData_To_Fifo(YY_ZD,8); // 指定语音
			}
			SYN7318_Get_String(Back,3);  //语音识别命令回传结果	
			if(Back[0] == 0xfc)        //帧头判断
			{
            ...
            case 0x06:  
						...
						default:
						{														  
							SYN7318_Put_String(Stop_ASR_Buf,4); 
							break;
						}
					}	
				}  
