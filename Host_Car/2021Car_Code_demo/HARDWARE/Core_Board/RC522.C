...
返 回 值：无
**/
uint8_t Read_Card(uint8_t Addr,unsigned char*RXRFID)
{
	...
	uint8_t s = Addr;       						  	
		...
		{
			status=MI_ERR;
			DEBUG_LOG("Find_Card OK!\n");
			status = PcdAnticoll(SN);				//防冲撞
			if(status == MI_OK)
			{
				status=MI_ERR;
				DEBUG_LOG("anti_collision OK!\n");		
				status =PcdSelect(SN);				//选定此卡
				if(status == MI_OK)					//选定成功
				{
					DEBUG_LOG("Choose_Card OK!\n");		
					status=MI_ERR;				
					status =PcdAuthState(0x60,s,KEY,SN);		//验证密钥
					if(status == MI_OK)
					{
						DEBUG_LOG("password OK!\n");		
						status = MI_ERR;						
						status=PcdRead(s,RXRFID);				//读卡
						if(status == MI_OK)
						{
							status = MI_ERR;				
							DEBUG_LOG("Read_Card OK!\n");	
							DEBUG_LOG("Card_Data:\n");	
								printf("%s\n",RXRFID);
...
void Write_Card(uint8_t Addr,uint8_t *data)
{
	signed char status = MI_ERR;
	uint8_t CT[2];									//卡类型
	uint8_t SN[4]; 									//卡号
...
{
	RC522_Uart_init(9600);	// 串口初始化为128000
	delay_ms(500);
	InitRc522();			//读卡器初始化
}
...
//用MF522计算CRC16函数
/////////////////////////////////////////////////////////////////////
signed char CalulateCRC(unsigned char *pIndata,unsigned char len,unsigned char *pOutData)
{
    unsigned char i,n;
...
//          mask[IN]:置位值
/////////////////////////////////////////////////////////////////////
signed char SetBitMask(unsigned char reg,unsigned char mask)  
{
    char tmp = 0x0;
    tmp = ReadRawRC(reg);
...
//          mask[IN]:清位值
/////////////////////////////////////////////////////////////////////
signed char ClearBitMask(unsigned char reg,unsigned char mask)  
{
    char tmp = 0x0;
    tmp = ReadRawRC(reg);
