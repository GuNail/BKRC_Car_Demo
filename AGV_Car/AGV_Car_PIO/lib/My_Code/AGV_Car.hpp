...
void _AGV_Car::Stop_Flag_Cheack()
{
    ...
    if(DCMotor.Stop_Flag==0x02)
    {
        DCMotor.Stop_Flag=0x00;
        delay(10);
        DCMotor.Go(80,80);
        delay(10);
    ...
    if(DCMotor.Stop_Flag==0x04)
    {
        DCMotor.Stop_Flag=0x00;
        delay(10);
        DCMotor.Find_Line(60);
        delay(10);
        DCMotor.CarTrack_Go(60);
        delay(10);
    }    
...
void _AGV_Car::Init()
{
    ...
    ExtSRAMInterface.Initialization();        //底层通信函数初始化
    LED.Initialization();                     //任务板LED灯初始化
    BH1750.Initialization();                  //光强度传感器初始化
    BEEP.Initialization();                   //任务板蜂鸣器初始化
    Infrare.Initialization();                 //红外通信初始化
    Ultrasonic.Initialization();             //超声波测距传感器初始化
    DCMotor.Initialization();                //电机控制初始化
    SYN7318.Initialization();                //语音识别初始化 
}
